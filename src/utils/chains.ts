...
const getInfuraUrlFor = (network: string): string | undefined =>
  process.env.infuraKey ? `https://${network}.infura.io/v3/${process.env.infuraKey}` : undefined;
const getAlchemyUrlFor = (network: string): string | undefined =>
...
export const TESTNET_CHAINS: ChainConfig = {
  5: {
    urls: [getInfuraUrlFor('goerli')].filter((url): url is string => !!url),
    name: 'Görli',
  },
  1337: {
  ...
  80001: {
    urls: [getInfuraUrlFor('polygon-mumbai')].filter((url): url is string => !!url),
    name: 'Polygon Mumbai',
    nativeCurrency: MATIC,
